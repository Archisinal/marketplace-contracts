import type * as EventTypes from '../event-types/mock_arch_nft';
...
export default class EventsClass {
	...
	public subscribeOnApprovalForAllEvent(callback : (event : EventTypes.ApprovalForAll) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('ApprovalForAll', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.ApprovalForAll);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'ApprovalForAll');
	}
